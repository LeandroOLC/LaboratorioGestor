...
using LaboratorioGestor.Controllers;
...
namespace LaboratorioGestor.App.Controllers
{
    ...
    public class AccountController : BaseController
    {
        ...
        public async Task<IActionResult> Login(LoginViewModel model, string returnUrl = null)
        {
            ...
            if (result.Succeeded)
            {
                _logger.LogInformation(1, "Usuario " + model.UserName + " logado com sucesso");
            ...
            {
                _logger.LogWarning(2, "Usuário temporariamente bloqueado por tentativas inválidas");
                return View("Lockout");
            }
            else
            {
        ...
        public async Task<IActionResult> Register(RegisterViewModel model, string returnUrl = null)
        {
            ...
            var user = new ApplicationUser { UserName = model.Nome, Email = model.Email, EmailConfirmed = true };

            var resultUser = await _userManager.CreateAsync(user, model.Password);
            
            if (resultUser.Succeeded)
            {
                var protetico = new ProteticoViewModel
                {
                    Id = Guid.Parse(user.Id),
                    Contatos = new ContatoViewModel { Email = user.Email, DataDoCadastro = DateTime.Now },
                    Enderecos = new EnderecoViewModel { DataDoCadastro = DateTime.Now },
                    Nome = model.Nome,
                    PercentualDaComissao = model.PercentualDaComissao,
                    CPF = model.CPF
                };

                var proteticoViewModel = _mapper.Map<Proteticos>(protetico);

                await _proteticoService.Adicionar(proteticoViewModel);
        ...
        public async Task<IActionResult> Logout()
        {
            ...
            _logger.LogInformation(4, "Usuário desconectado.");
